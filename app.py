...
def user(username):
    ...
    try:
        # token을 시크릿키로 디코딩합니다.
        # 우리가 로그인 시 넣은 그 payload와 같은 것이 나옵니다.
        payload = jwt.decode(token_receive, SECRET_KEY, algorithms=['HS256'])
...
def sign_in():
    ...
    # 회원가입 때와 같은 방법으로 pw를 암호화합니다.(db에는 암호화된 상태로 저장되어있기 때문에)
...
def sign_up():
    ...
    doc = {
        ...
        "profile_pic_real": "/images/profile_placeholder.png",      # 프로필 사진 기본 이미지
...
def save_img():
    ...
    try:
        ...
        if 'file_give' in request.files:
            ...
            file_path = f"upload/{username}.{extension}"
